...
当前应优先推进的最早未完成任务：M11-1 实验记录 API。
...
| ID | 任务 | 状态 | 备注 |
| --- | --- | --- | --- |
| M11-1 | 添加实验记录 API | Not Started | 读取参数实验列表和详情，遵守 data + meta 响应约定 |
| M11-2 | 添加实验记录页面 | Not Started | 展示实验列表、参数、指标、备注和空状态 |
| M11-3 | 添加通知历史 API | Not Started | 读取通知列表，支持通道过滤和数量限制 |
| M11-4 | 添加通知历史页面 | Not Started | 展示通知通道、级别、状态、时间和元数据摘要 |
| M11-5 | 添加券商准入评估 API | Not Started | 返回准入状态、失败原因和只读动作边界 |
| M11-6 | 添加券商准入评估页面 | Not Started | 明确展示不自动下单、准入条件和失败原因 |
