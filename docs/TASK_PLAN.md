...
| M12-2 | 增加数据更新任务入口 | Done | 已新增 `POST /api/data/update`，支持按数据类型手动触发本地 CSV/AkShare 更新，并写入运行日志 |
| M12-3 | 增加数据更新日志 API | Done | 已新增 `GET /api/data/update-logs`，支持查询数据更新任务状态、错误、结果摘要、状态过滤和数量限制 |
| M12-4 | 增加数据更新日志页面 | Done | 已在数据状态页展示最近数据更新日志，包含状态筛选、数据类型、数据源、更新数量和错误信息 |
| M12-5 | 增加数据质量检查报告 | Done | 已新增 `GET /api/data/quality-report`，检查数据新鲜度、缺失字段、异常字段和财务披露日期风险 |
| M12-6 | 增加策略运行前置检查 | Done | 已新增 `GET /api/strategy/preflight`，并在手动运行本周策略前执行检查；存在 error 时阻止运行，warning 时允许运行并返回风险提示 |
| M12-7 | 增加一键生成本周完整报告流程 | Done | 已新增 `POST /api/reports/weekly-full`，串联前置检查、调仓建议、HTML/CSV/Excel 报告摘要和本地通知；前置检查失败时停止并写入失败日志 |
